...
public class EffectiveSynergyCalculatorDiscrete
  ...
  public EffectiveSynergyCalculatorDiscrete(int base, int dimensions) {
  ...
  public double computeForPartition(List<List<Integer>> partition) {
    ...
    try {
      ...
      int rBase = (int)(Math.pow(base, dimensions));
      int[] allvars = MatrixUtils.range(0, dimensions-1);
      ...
      for (int i = 0; i < partition.size(); i++) {
        ...
        double[][] partJoint = MatrixUtils.marginaliseJointPDF(jointPDF, p, allvars, base, dimensions);

        MutualInformationCalculatorDiscrete micd;
        micd = new MutualInformationCalculatorDiscrete(partBase, rBase, 0);
        sourceMI[i] = micd.computeFromJointPDF(partJoint);
      }

      // Subtract sum of MI of partitions from the MI of system.
      rvalue = systemInformation - MatrixUtils.max(sourceMI);
